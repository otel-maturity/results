...
def normalize_card_href(project, card_text):
    """Rewrite the project-card href to the canonical published path.

    Each results/<project>/ contains report.html and project-card.html.
    The sync-reports workflow flattens report.html into
    site/<project>-report.html, so the card's href must point at
    `<project>-report.html`. Whatever the generator wrote (`report.html`,
    `<project>/report.html`, etc.) we normalize here so the pipeline
    is tolerant of the generator's output.

    Returns (new_text, error_or_None). An error is only returned when
    the card has no <a class="project-card" href=...> at all.
    """
    m = CARD_HREF_RE.search(card_text)
    if not m:
        return card_text, f"{project}: no <a class=\"project-card\" href=...> found"
    expected = f"{project}-report.html"
    href_start, href_end = m.span(1)
    if m.group(1) == expected:
        return card_text, None
    new_text = card_text[:href_start] + expected + card_text[href_end:]
...
def merge(results_dir, projects_html):
    ...
    for project, card_path in projects:
        ...
        normalized, err = normalize_card_href(project, card_text)
        if err:
            errors.append(err)
            continue
        if normalized != card_text:
            print(f"normalized href: {project} -> {project}-report.html")
        cards.append((project, normalized))
    ...
    for project, card_text in cards:
        new_card = card_text.strip()
        if not new_card:
